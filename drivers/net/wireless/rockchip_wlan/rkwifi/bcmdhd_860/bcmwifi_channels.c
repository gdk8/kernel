...
char *wf_chspec_ntoa(chanspec_t chspec, char *buf)
{
	...
	if (CHSPEC_IS20(chspec)) {
		...
		/* 80+80 */
		uint chan1 = (chspec & WL_CHANSPEC_CHAN1_MASK) >>
			     WL_CHANSPEC_CHAN1_SHIFT;
		uint chan2 = (chspec & WL_CHANSPEC_CHAN2_MASK) >>
			     WL_CHANSPEC_CHAN2_SHIFT;

		/* convert to channel number */
		chan1 = (chan1 < WF_NUM_5G_80M_CHANS) ? wf_5g_80m_chans[chan1] :
							      0;
		chan2 = (chan2 < WF_NUM_5G_80M_CHANS) ? wf_5g_80m_chans[chan2] :
							      0;

		/* Outputs a max of CHANSPEC_STR_LEN chars including '\0'  */
		snprintf(buf, CHANSPEC_STR_LEN, "%d/80+80/%d-%d", pri_chan,
			 chan1, chan2);
	}
...
chanspec_t wf_chspec_aton(const char *a)
{
	...
	if (c == 'g') {
		...
		pri_ch = num;
		chspec_band =
			((pri_ch <= CH_MAX_2G_CHANNEL) ? WL_CHANSPEC_BAND_2G :
							       WL_CHANSPEC_BAND_5G);
	}
...
bool wf_chspec_coexist(chanspec_t chspec1, chanspec_t chspec2)
{
	...
	if (same_primary && CHSPEC_IS2G(chspec1)) {
		if (CHSPEC_IS40(chspec1) && CHSPEC_IS40(chspec2)) {
			return (CHSPEC_CTL_SB(chspec1) ==
				CHSPEC_CTL_SB(chspec2));
		}
	}
	return same_primary;
}
...
chanspec_t wf_create_20MHz_chspec(uint channel, chanspec_band_t band)
{
	...
	if (channel <= WL_CHANSPEC_CHAN_MASK &&
		...
		chspec = band | WL_CHANSPEC_BW_20 | WL_CHANSPEC_CTL_SB_NONE |
			 channel;
...
extern chanspec_t wf_chspec_primary40_chspec(chanspec_t chspec)
{
	...
	if (CHSPEC_IS80(chspec)) {
		...
		chspec40 = (chanspec_t)(WL_CHANSPEC_BAND_5G |
					WL_CHANSPEC_BW_40 | sb | center_chan);
	}
...
chanspec_t wf_chspec_primary80_chspec(chanspec_t chspec)
{
	...
	if (CHSPEC_IS80(chspec)) {
		...
		chspec80 = (chanspec_t)(WL_CHANSPEC_BAND_5G |
					WL_CHANSPEC_BW_80 | sb | center_chan);
...
chanspec_t wf_chspec_secondary80_chspec(chanspec_t chspec)
{
	...
	if (CHSPEC_IS8080(chspec)) {
		...
		chspec80 =
			(chanspec_t)(WL_CHANSPEC_BAND_5G | WL_CHANSPEC_BW_80 |
...
void wf_get_all_ext(chanspec_t chspec, uint8 *pext)
{
	...
	chanspec_t t = (CHSPEC_IS160(chspec) ||
			CHSPEC_IS8080(chspec)) ? /* if bw > 80MHz */
	...
	/* 20MHz EXT */
	(pext)[1] = pri_ch + (uint8)(IS_CTL_IN_L20(t) ? CH_20MHZ_APART :
							      -CH_20MHZ_APART);
	if (CHSPEC_IS40(chspec))
